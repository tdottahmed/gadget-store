...
@section('content')
  ...
  <div class="content container-fluid">
    ...
    <div class="row py-3">
      @if ($fraudCheckHistory == null)
        {{-- No fraud history yet --}}
        <div class="col-12">
          <div class="card text-center">
            <div class="card-body">
              <h5 class="card-title mb-3">{{ translate('No Fraud Check Found') }}</h5>
              <p class="card-text mb-3">
                {{ translate('This customer has no recorded fraud check history yet. You can run a fraud check to see if there are any suspicious activities or order anomalies.') }}
              </p>
              <a href="{{ route('admin.orders.fraud-check', [$order['id']]) }}" class="btn btn-primary px-4">
                {{ translate('Run Fraud Check') }}
              </a>
            </div>
          </div>
        </div>
          ...
          <div class="col-12">
            <div class="card shadow-sm">
              <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">{{ translate('Fraud Check & History') }}</h5>
                <a href="{{ route('admin.orders.fraud-check', [$order['id']]) }}"
                   class="btn btn-primary px-4">{{ translate('Re-run Fraud Check') }}</a>
              </div>
              <div class="card-body">
                <div class="row text-center">
                  <div class="col-md-3 col-12">
                    <div class="rounded border p-3">
                      <p class="mb-1">{{ translate('Total Orders') }}:
                        <strong>{{ $fraudCheckHistory->total_orders }}</strong>
                      </p>
                      <p class="text-success mb-1">{{ translate('Success Orders') }}:
                        <strong>{{ $fraudCheckHistory->success_orders }}</strong>
                      </p>
                      <p class="text-danger mb-0">{{ translate('Cancelled Orders') }}:
                        <strong>{{ $fraudCheckHistory->cancelled_orders }}</strong>
                      </p>
                      <p class="mb-0">{{ translate('Success Rate') }}:
                  ...
                  <div class="col-md-3 col-12 mb-md-0 mb-3">
                    <div class="rounded border p-3">
                      <h6>{{ translate('Steadfast') }}</h6>
                      <p class="mb-1">{{ translate('Total Orders') }}:
                        <strong>{{ $fraudCheckHistory->steadfast_total_orders }}</strong>
                      </p>
                      <p class="text-success mb-1">{{ translate('Success') }}:
                        <strong>{{ $fraudCheckHistory->steadfast_success_order }}</strong>
                      </p>
                      <p class="text-danger mb-0">{{ translate('Cancelled') }}:
    ...
    <div class="row gy-3" id="printableArea">
      <div class="col-lg-8">
        <div class="card h-100">
          <div class="card-body">
            <div class="d-flex flex-md-nowrap justify-content-between mb-4 flex-wrap gap-10">
              <div class="d-flex flex-column gap-10">
                ...
                <div class="d-flex flex-wrap gap-3">
                  @if ($linkedOrders->count() > 0)
                    <div class="color-caribbean-green-soft fw-bold d-flex align-items-center rounded px-2 py-1">
                      {{ translate('linked_orders') }}
                      ({{ $linkedOrders->count() }}) :
                    </div>
                    @foreach ($linkedOrders as $linked)
                      <a href="{{ route('admin.orders.details', [$linked['id']]) }}"
                         class="color-caribbean-green rounded px-2 py-1 text-white">{{ $linked['id'] }}</a>
                    @endforeach
                  @endif
                  @if ($order['payment_method'] == 'cash_on_delivery' && $order['bring_change_amount'] > 0)
                    <div
                         class="bg-success fs-12 text-dark d-flex align-items-center gap-2 rounded bg-opacity-10 px-12 py-10">
                      <div>
              ...
              <div class="text-sm-end flex-grow-1">
                <div class="d-flex justify-content-start justify-content-lg-end flex-wrap gap-10">
                  ...
                  @if (getWebConfig('map_api_status') == 1 && isset($shippingAddress->latitude) && isset($shippingAddress->longitude))
                    <div class="">
                      <button class="btn btn-primary px-4" data-bs-toggle="modal" data-bs-target="#locationModal">
                  ...
                  <a class="btn btn-primary px-4" target="_blank"
                    ...
                    <img src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/icons/uil_invoice.svg') }}"
                         alt="" class="mr-1">
                    {{ translate('print_Invoice') }}
                  </a>
                </div>
                <div class="d-flex flex-column mt-3 gap-2">
                  <div class="order-status d-flex justify-content-sm-end text-capitalize gap-10">
                    <span class="text-dark">{{ translate('status') }}: </span>
                    @if ($order['order_status'] == 'pending')
                      <span
                            class="badge color-caribbean-green-soft fw-bold rounded-50 d-flex align-items-center px-2 py-1">{{ translate(str_replace('_', ' ', $order['order_status'])) }}</span>
                    @elseif($order['order_status'] == 'failed')
                      <span
                            class="badge badge-danger text-bg-danger fw-bold rounded-50 d-flex align-items-center px-2 py-1">{{ translate(str_replace('_', ' ', $order['order_status'] == 'failed' ? 'Failed to Deliver' : '')) }}
                      </span>
                    @elseif($order['order_status'] == 'processing' || $order['order_status'] == 'out_for_delivery')
                      <span
                            class="badge badge-warning text-bg-warning fw-bold rounded-50 d-flex align-items-center px-2 py-1">
                        {{ translate(str_replace('_', ' ', $order['order_status'] == 'processing' ? 'Packaging' : $order['order_status'])) }}
                      </span>
                    @elseif($order['order_status'] == 'delivered' || $order['order_status'] == 'confirmed')
                      <span
                            class="badge badge-success text-bg-success fw-bold rounded-50 d-flex align-items-center px-2 py-1">
                        {{ translate(str_replace('_', ' ', $order['order_status'])) }}
                      </span>
                    @else
                      <span
                            class="badge badge-danger text-bg-danger fw-bold rounded-50 d-flex align-items-center px-2 py-1">
                        {{ translate(str_replace('_', ' ', $order['order_status'])) }}
                      </span>
                    @endif
                  </div>

                  <div class="payment-method d-flex justify-content-sm-end text-capitalize gap-10">
                  ...
                  @if (
                    ...
                    <div class="reference-code d-flex justify-content-sm-end text-capitalize gap-10">
            ...
            @if ($order->order_note != null)
              <div class="w-100 d-block mb-5 mt-2">
                <div class="gap-10">
            ...
            <div class="table-responsive">
              <table class="fs-12 table-hover table-borderless align-middl table">
                      ...
                      <tr>
                        ...
                        <td>
                          <div class="media align-items-center gap-10">
                            <img class="avatar avatar-60 img-fit rounded"
                      ...
                      <div class="modal fade" id="fileUploadModal-{{ $detail->id }}" tabindex="-1"
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                          <div class="modal-content">
                            <form action="{{ route('admin.orders.digital-file-upload-after-sell') }}" method="post"
                              ...
                              <div class="modal-body">
                                @if ($detail?->digital_file_after_sell_full_url && isset($detail->digital_file_after_sell_full_url['key']))
                                  <div class="mb-4">
                                    ...
                                    <span data-file-path="{{ $downloadPathExist ? $detail->digital_file_after_sell_full_url['path'] : 'javascript:' }}"
                                ...
                                @elseif($detail->digital_file_after_sell)
                                  <div class="mb-4">
                                    ...
                                    <span data-file-path="{{ file_exists($downloadPath) ? $downloadPath : 'javascript:' }}"
                                ...
                                @else
                                  <h4 class="mt-3 text-center">
                                    {{ translate('admin_have_no_permission_for_vendors_digital_product_upload') }}</h4>
                                @endif
                              </div>
            ...
            <div class="px-sm-5 overflow-x-auto">
              <table class="table-borderless table-sm text-sm-right mb-0 table text-nowrap">
                <tbody>
                  <tr>
                    <td class="text-body text-capitalize text-end"><strong>{{ translate('item_price') }}</strong></td>
                  ...
                  <tr>
                    <td class="text-body text-capitalize text-end"><strong>{{ translate('item_discount') }}</strong>
                    </td>
                    <td class="text-dark text-end">
                      -
                  ...
                  <tr>
                    <td class="text-body text-capitalize text-end"><strong>{{ translate('sub_total') }}</strong></td>
                    <td class="text-dark text-end">
                  ...
                  <tr>
                    <td class="text-body text-end"><strong>{{ translate('coupon_Discount') }}</strong></td>
                    <td class="text-dark text-end">
                      <strong>-
                  ...
                  @if ($orderTotalPriceSummary['referAndEarnDiscount'] > 0)
                    <tr>
                      <td class="text-body text-end"><strong>{{ translate('referral_discount') }}</strong></td>
                      <td class="text-dark text-end">
                        <strong>-
                  ...
                  @endif
                  <tr>
                    <td class="text-body text-uppercase text-end">
                      <strong>{{ translate('vat') }}/{{ translate('tax') }}</strong>
                    </td>
                    <td class="text-dark text-end">
                  ...
                  <tr>
                    <td class="text-body text-capitalize text-end">
                  ...
                  <tr>
                    <td class="text-body text-end"><strong>{{ translate('total') }}</strong></td>
                    <td class="text-dark text-end">
                  ...
                  @if ($order->order_type == 'pos' || $order->order_type == 'POS')
                    <tr>
                      <td class="text-body text-end"><strong>{{ translate('paid_amount') }}</strong></td>
                      <td class="text-dark text-end">
                    ...
                    <tr>
                      <td class="text-body text-end"><strong>{{ translate('change_amount') }}</strong></td>
                      <td class="text-dark text-end">
      ...
      <div class="col-lg-4 d-flex flex-column gap-3">
        ...
        <div class="card">
          <div class="card-body text-capitalize d-flex flex-column gap-4">
            ...
            <div class="d-flex justify-content-between align-items-center form-control h-auto flex-wrap gap-10">
              ...
              <div class="d-flex justify-content-end min-w-100 align-items-center gap-2">
                ...
                <label class="switcher payment-status-text">
                  ...
                  <span
                        class="switcher_control switcher_control_add {{ $order->payment_status == 'paid' ? 'checked' : 'unchecked' }}"></span>
                </label>
              </div>
            </div>
            @if ($physicalProduct)
              ...
              <ul class="list-unstyled list-unstyled-py-4">
                <li class="form-group">
                  ...
                  <div class="select-wrapper">
                    <select class="form-select" name="delivery_type" id="choose_delivery_type"
                      ...
                      <option value="0" selected>{{ translate('choose_delivery_type') }}</option>
                      <option value="stead_fast_courier"
                              {{ $order->delivery_type == 'stead_fast_courier' ? 'selected' : '' }}>
                        {{ translate('Stead fast Courier') }}
                      </option>
                      ...
                      <option value="third_party_delivery"
                        ...
                        {{ translate('Other Third Party Delivery') }}
                      </option>
                    </select>
                  </div>
                ...
                <li class="choose_delivery_man form-group">
                  ...
                  <select class="custom-select" name="delivery_man_id" id="addDeliveryMan"
                          data-order-id="{{ $order['id'] }}" data-placeholder="Select from dropdown"
                          {{ $order->order_status == 'delivered' ? 'disabled' : '' }}>
                    <option></option>
                ...
                @if (isset($order->deliveryMan))
                  <li class="choose_delivery_man form-group">
                    <label class="form-label fw-semibold">
                      ...
                      <span class="tooltip-icon cursor-pointer" data-bs-toggle="tooltip" data-bs-placement="top"
                    ...
                    <div class="d-flex align-items-center gap-2">
                      ...
                      <button
                              class="btn btn-primary {{ $order['order_status'] == 'delivered' ? 'disabled deliveryman-charge-alert' : 'deliveryman-charge' }} h-40">{{ translate('update') }}</button>
                    </div>
                  </li>
                  <li class="choose_delivery_man form-group">
                    <label class="form-label fw-semibold">{{ translate('expected_delivery_date') }}</label>
                ...
                <li class="form-group mt-1" id="by_third_party_delivery_service_info">
        ...
        @if (!$order->is_guest && $order->customer)
          <div class="card">
            <div class="card-body">
              <div class="d-flex align-items-center justify-content-between mb-4 gap-2">
                <h3 class="d-flex fw-semibold gap-2">
              ...
              <div class="media flex-wrap gap-3">
                ...
                <div class="media-body d-flex flex-column gap-1">
                  ...
                  <span class="text-dark"> <span class="fw-semibold">{{ $orderCount }}</span>
                    {{ translate('orders') }}</span>
                  <span class="text-dark break-all"><span
              ...
              <div class="card-body">
                <div class="d-flex align-items-center justify-content-between mb-4 gap-2">
                  ...
                  @if ($order['order_status'] != 'delivered')
                    <button class="btn btn-outline-primary icon-btn" title="Edit" data-bs-toggle="modal"
                ...
                <div class="d-flex flex-column gap-2">
                  <div>
                    ...
                    <span class="fw-semibold">{{ $shippingAddress->contact_person_name }}</span>
          ...
          <div class="card">
            <div class="card-body">
              <div class="d-flex align-items-center justify-content-between mb-4 gap-2">
                ...
                @if ($order['order_status'] != 'delivered')
                  <button class="btn btn-outline-primary icon-btn billing-address-update-modal"
              ...
              <div class="d-flex flex-column gap-2">
                <div>
                  <span>{{ translate('name') }} :</span>
                  <span class="fw-semibold">{{ $billing->contact_person_name }}</span>
                  {{ $order->is_guest ? '(' . translate('guest_customer') . ')' : '' }}
                </div>
                <div>
                  <span>{{ translate('contact') }} :</span>
          ...
          <div class="card-body">
            ...
            <div class="media d-flex align-items-center gap-3">
              @if ($order->seller_is == 'admin')
                <div class="me-3">
                  <img class="avatar avatar-70 img-fit-contain rounded"
                       src="{{ getStorageImages(path: getWebConfig(name: 'company_fav_icon'), type: 'shop') }}"
                       alt="">
                </div>
                <div class="media-body d-flex flex-column gap-2">
                  <h5>{{ $companyName }}</h5>
                  <span class="text-dark"><strong>{{ $totalDelivered }}</strong>
                    {{ translate('orders_Served') }}</span>
                </div>
              @else
                @if (!empty($order->seller->shop))
                  <div class="me-3">
                    <img class="avatar avatar-70 img-fit rounded"
      ...
      <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
          <div class="modal-header d-flex justify-content-between align-items-center pb-4">
            ...
            <button type="button" class="btn-close btn-circle bg-section2 border-0 shadow-none"
                    data-bs-dismiss="modal" aria-label="Close">
            </button>
          </div>
          <div class="modal-body px-sm-5 px-4 pt-0">
            <div class="d-flex flex-column align-items-center gap-2">
              <div class="row gx-2">
                @foreach ($order->verificationImages as $image)
                  <div class="col-lg-4 col-sm-6">
                    <div class="border-1 mb-2 mt-2">
  ...
  <div class="modal fade" id="shippingAddressUpdateModal" tabindex="-1" aria-labelledby="shippingAddressUpdateModal"
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
      <div class="modal-content">
        <div class="modal-header d-flex justify-content-between align-items-center border-0 pb-4">
          <h3 class="mb-0">{{ translate('shipping_address') }}</h3>
          <button type="button" class="btn-close btn-circle bg-section2 border-0 shadow-none" data-bs-dismiss="modal"
              ...
              <div class="row gx-3 gy-4">
                <div class="col-md-6">
                  <div class="form-group">
                    <label for="name" class="form-label">{{ translate('contact_person_name') }}</label>
                ...
                <input type="hidden" id="latitude" name="latitude" class="form-control d-inline"
                       placeholder="{{ translate('Ex') }} : -94.22213" value="{{ $shippingAddress->latitude ?? 0 }}"
                       required readonly>
                ...
                @if (getWebConfig('map_api_status') == 1)
                  <div class="col-12">
                    <input id="pac-input" class="form-control w-200 mt-1 rounded"
                           title="{{ translate('search_your_location_here') }}" type="text"
                           placeholder="{{ translate('search_here') }}" />
                    <div class="dark-support w-100 h-200 mb-2 rounded" id="location_map_canvas_shipping"></div>
                  </div>
                @endif
                <div class="col-12">
                  <div class="d-flex justify-content-end gap-3">
                  ...
                  <div class="row gx-3 gy-4">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        @else
                          <input type="text" class="form-control" value="{{ $billing ? $billing->zip : '' }}"
                    ...
                    @if (getWebConfig('map_api_status') == 1)
                      <div class="col-12">
                        <input id="billing-pac-input" class="form-control w-200 mt-1 rounded"
                               title="{{ translate('search_your_location_here') }}" type="text"
                               placeholder="{{ translate('search_here') }}" />
                        <div class="w-100 h-200 mb-2 rounded" id="location_map_canvas_billing"></div>
                      </div>
                    @endif
                    <div class="col-12">
                      <div class="d-flex justify-content-end gap-3">
    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        <div class="modal-header d-flex justify-content-end border-0 pb-0">
          <h3 class="modal-title flex-grow-1 text-center" id="locationModalLabel">{{ translate('location_on_Map') }}
          </h3>
          <button type="button" class="btn-close btn-circle bg-section2 border-0 shadow-none" data-bs-dismiss="modal"
                  aria-label="Close">
          </button>
          ...
          <div>
            <div class="row">
              <div class="col-md-12 rounded border p-3">
                ...
                <ul class="nav nav-tabs media-tabs nav-justified order-track-info border-0">
                  <li class="nav-item">
                      ...
                      <div class="d-flex flex-sm-column gap-sm-0 gap-3">
                        ...
                        <div class="media-body">
                          <div class="text-sm-center text-start">
                            <h6 class="media-tab-title text-capitalize fs-14 mb-0 text-nowrap">
                      ...
                      <li class="nav-item">
                        <div
                          ...
                          <div class="d-flex flex-sm-column gap-sm-0 gap-3">
                            ...
                            <div class="media-body">
                              <div class="text-sm-center text-start">
                                <h6 class="media-tab-title fs-14 mb-0 text-nowrap">
                      ...
                      <li class="nav-item">
                        <div class="nav-link {{ $order['order_status'] == 'delivered' ? 'active-status' : '' }}">
                          <div class="d-flex flex-sm-column gap-sm-0 gap-3">
                            ...
                            <div class="media-body">
                              <div class="text-sm-center text-start">
                                <h6 class="media-tab-title fs-14 mb-0 text-nowrap">{{ translate('order_Shipped') }}
                                </h6>
                              </div>
                      ...
                      <li class="nav-item">
                        <div
                          ...
                          <div class="d-flex flex-sm-column gap-sm-0 gap-3">
                            ...
                            <div class="media-body">
                              <div class="text-sm-center text-start">
                                <h6 class="media-tab-title fs-14 mb-0 text-nowrap">{{ translate('delivery_complete') }}
                                </h6>
                              </div>
