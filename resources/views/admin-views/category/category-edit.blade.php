        ...
        <div class="card">
          ...
          <div class="card-body text-start">
            <form action="{{ route('admin.category.update', [$category['id']]) }}" method="POST"
              ...
              @csrf
              <div class="table-responsive mb-4 w-auto overflow-y-hidden">
                <div class="position-relative nav--tab-wrapper">
                    ...
                    @foreach ($languages as $lang)
                      <li class="nav-item px-0">
                        <a data-bs-toggle="pill" data-bs-target="#{{ $lang }}-form" role="tab"
                           class="nav-link {{ $lang == $defaultLanguage ? 'active' : '' }} px-2"
                           id="{{ $lang }}-link">
                  ...
                  <div class="nav--tab__prev">
                    <button class="btn btn-circle text-primary border-0 bg-white">
              ...
              <div class="row mb-4">
                <div class="{{ $category['parent_id'] == 0 || $category['position'] == 1 ? 'col-lg-6' : 'col-12' }}">
                  <div class="bg-section rounded-8 w-100 h-100 p-20">
                    ...
                    <div class="form-group">
                      <label class="form-label text-capitalize" for="priority">{{ translate('priority') }}</label>
                      <div class="select-wrapper">
                        <select class="form-select" name="priority" id="" required>
                          @for ($index = 0; $index <= 10; $index++)
                            <option value="{{ $index }}" {{ $category['priority'] == $index ? 'selected' : '' }}>
                              {{ $index }}</option>
                          @endfor
                        </select>
                      </div>
                ...
                @if ($category['parent_id'] == 0 || ($category['position'] == 1 && theme_root_path() == 'theme_aster'))
                  <div class="col-lg-6 mt-lg-0 from_part_2 mt-4">
                ...
                @if ($category['position'] == 2 || ($category['position'] == 1 && theme_root_path() != 'theme_aster'))
                  <div class="d-flex justify-content-end mt-4 flex-wrap gap-3">
