...
@section('content')
      ...
      <div class="d-flex flex-column gap-3">
        <div class="card">
          <div class="card-body d-flex flex-column gap-sm-20 gap-3">
            <div class="p-sm-20 bg-section rounded p-12">
                ...
                <div>
                  <label class="switcher" for="customer-wallet-status">
                    <input class="switcher_input custom-modal-plugin" type="checkbox" value="1"
                           name="customer_wallet" id="customer-wallet-status" {{ $walletStatus == 1 ? 'checked' : '' }}
              ...
              <div class="row g-4">
                ...
                <div class="col-xl-4 col-md-6">
                  <div class="form-group">
                    ...
                    <label
                           class="d-flex justify-content-between align-items-center user-select-none gap-3 rounded border bg-white px-3 py-10">
                      <span class="fw-medium text-dark">{{ translate('status') }}</span>
                      <label class="switcher" for="add-funds-to-wallet">
                        <input class="switcher_input custom-modal-plugin" type="checkbox" value="1"
                               name="add_funds_to_wallet" id="add-funds-to-wallet"
                               {{ $walletStatus && $addFundsToWallet ? 'checked' : '' }}
                               {{ $walletStatus == 0 ? 'disabled' : '' }} data-modal-type="input-change"
                ...
                <div class="col-xl-4 col-md-6">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="loyalty_point_exchange_rate"
                           {{ $loyaltyPointStatus == 0 ? '' : 'required' }} id="loyalty_point_exchange_rate"
                           placeholder="{{ translate('ex') . ': ' . '10' }}"
              ...
              <div class="row g-4 align-items-center">
                  ...
                  <div>
                    <label class="switcher" for="customer-loyalty-point-each-order">
                      <input class="switcher_input custom-modal-plugin" type="checkbox" value="1"
                             ...
                             {{ $loyaltyPointStatus ? '' : 'disabled' }} data-modal-type="input-change"
                             data-on-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/loyalty-on.png') }}"
                             data-off-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/loyalty-off.png') }}"
                ...
                <div class="col-12">
                  ...
                  <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="item_purchase_point" id=""
        ...
        <div class="card mb-3">
          <div class="card-body d-flex flex-column gap-sm-20 gap-3">
              ...
              <div class="d-flex justify-content-between align-items-center gap-3">
                ...
                <div>
                  <label class="switcher" for="ref-earning-status">
                    <input class="switcher_input custom-modal-plugin" type="checkbox" value="1"
                           name="ref_earning_status" id="ref-earning-status" {{ $refEarningStatus ? 'checked' : '' }}
                           data-modal-type="input-change"
            ...
            @php($minAmount = getWebConfig(name: 'affiliate_withdrawal_minimum_amount'))
            @php($maxAmount = getWebConfig(name: 'affiliate_withdrawal_maximum_amount'))
            @php($refEarningWithdrawalStatus = getWebConfig(name: 'affiliate_withdrawal_status'))
            <div class="p-sm-20 bg-section rounded p-12">
              <div class="row g-4 align-items-start">

                <div class="col-lg-4">
                  <h2>{{ translate('Withdrawal_Limits') }}</h2>
                  <p class="text-muted">
                    {{ translate('Define_the_minimum_and_maximum_amount_users_can_withdraw_from_their_affiliate_wallet.') }}
                  </p>
                  <div>
                    <label class="switcher" for="ref-earning-withdrawal-status">
                      <input class="switcher_input custom-modal-plugin" type="checkbox" value="1"
                             name="affiliate_withdrawal_status" id="ref-earning-withdrawal-status"
                             {{ $refEarningWithdrawalStatus ? 'checked' : '' }} data-modal-type="input-change"
                             data-on-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/referral-earning-on.png') }}"
                             data-off-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/referral-earning-off.png') }}"
                             data-on-title="{{ translate('want_to_Turn_ON_Referral_And_Earning_withdrawal_option') }}?"
                             data-off-title="{{ translate('want_to_Turn_OFF_Referral_And_Earning_withdrawal_option') }}?"
                             data-off-message="<p>{{ translate('if_disable_this_customer_will_not_be_able_to_withdraw_their_affiliate_rewards_from_the_affiliate_withdrawal_account') }}</p>">
                      <span class="switcher_control"></span>
                    </label>
                  </div>
                </div>
                ...
                <div class="col-lg-8">
                  ...
                  <div class="form-group mb-3">
                    <label class="form-label" for="affiliate_withdrawal_minimum_amount">
                      {{ translate('Minimum_Withdrawal_Amount') }}
                      ({{ getCurrencySymbol('default') }})
                    </label>

                    <input type="number" class="form-control" id="affiliate_withdrawal_minimum_amount"
                           name="affiliate_withdrawal_minimum_amount" placeholder="{{ translate('ex') }}: 10"
                           value="{{ Convert::default($minAmount) ?? 0 }}" min="0" step="0.01">

                    <small class="text-muted d-block mt-1">
                      {{ translate('users_cannot_submit_a_withdrawal_request_below_this_amount') }}
                    </small>
                  </div>

                  {{-- Maximum Withdrawal --}}
                  <div class="form-group mb-3">
                    <label class="form-label" for="affiliate_withdrawal_maximum_amount">
                      {{ translate('Maximum_Withdrawal_Amount') }}
                      ({{ getCurrencySymbol('default') }})
                    </label>

                    <input type="number" class="form-control" id="affiliate_withdrawal_maximum_amount"
                           name="affiliate_withdrawal_maximum_amount" placeholder="{{ translate('ex') }}: 500"
                           value="{{ Convert::default($maxAmount) ?? 0 }}" min="0" step="0.01">

                    <small class="text-muted d-block mt-1">
                      {{ translate('users_cannot_request_more_than_this_amount_in_a_single_withdrawal') }}
                    </small>
                  </div>
                </div>
              </div>
            ...
            <div class="row g-4 align-items-center">
              ...
              <div class="col-lg-8">
                <div class="row g-4">
                      ...
                      <label
                        ...
                        <label class="switcher" for="ref_earning_discount_status">
                          ...
                          <input class="switcher_input custom-modal-plugin" type="checkbox"
                                 @if (isset($referralEarningDiscountData['ref_earning_discount_status']) &&
                                         $referralEarningDiscountData['ref_earning_discount_status'] == 1) checked @endif value="1"
                                 name="ref_earning_discount_status" id="ref_earning_discount_status"
                                 data-modal-type="input-change"
                                 data-on-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/referral-earning-on.png') }}"
                                 data-off-image="{{ dynamicAsset(path: 'public/assets/new/back-end/img/modal/referral-earning-off.png') }}"
                  ...
                  <div class="col-lg-6">
                    <div class="form-group">
                      <label class="form-label" for="">{{ translate('Discount_Amount') }}
                        <span class="tooltip-icon" data-bs-toggle="tooltip" data-bs-placement="right"
                              data-bs-title="{{ translate('the_value_of_the_discount_the_referred_customer_will_get_on_their_first_order_.') }} {{ translate('you_can_choose_a_fixed_amount_or_a_percentage_.') }}">
                          <i class="fi fi-sr-info"></i>
                        </span>
                      </label>
                        ...
                        <div class="input-group-append select-wrapper">
                          <select name="discount_type" class="form-select shadow-none">
                            <option value="percentage"
                                    {{ isset($referralEarningDiscountData['discount_type']) && $referralEarningDiscountData['discount_type'] == 'percentage' ? 'selected' : '' }}>
                              %</option>
                            <option
                                    value="flat"{{ isset($referralEarningDiscountData['discount_type']) && $referralEarningDiscountData['discount_type'] == 'flat' ? 'selected' : '' }}>
                              $</option>
                          </select>
                  ...
                  <div class="col-lg-6">
                    <div class="form-group">
                      <label class="form-label" for="">{{ translate('Validity') }}
                        <span class="tooltip-icon" data-bs-toggle="tooltip" data-bs-placement="right"
                              data-bs-title="{{ translate('sets_how_long_the_referral_discount_will_remain_active_after_the_customer_signs_up_measured_in_days.') }}">
                          <i class="fi fi-sr-info"></i>
                        </span>
                      </label>
                      <div class="input-group">
                        <input type="number" name="validity" id="validity"
                               value="{{ $referralEarningDiscountData['validity'] ?? '' }}" class="form-control"
                               placeholder="{{ translate('Ex: 10') }}" min="1">
                        <div class="input-group-append select-wrapper">
                          <?php
  ...
  <div class="d-flex justify-content-end trans3 mt-4">
    <div
         class="d-flex justify-content-sm-end justify-content-center flex-grow-1 flex-grow-sm-0 action-btn-wrapper trans3 gap-3 bg-white">
      <button type="reset" class="btn btn-secondary px-sm-4 w-120 px-3">{{ translate('reset') }}</button>
      <button type="submit" class="btn btn-primary px-sm-4 px-3">
