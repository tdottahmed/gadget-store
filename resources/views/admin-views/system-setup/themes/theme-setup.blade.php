...
@section('content')
  <div class="content container-fluid">
    ...
    <h1 class="mb-sm-20 mb-3">
      {{ translate('Theme_Setup') }}
    </h1>

    <div class="card mb-3">
      <div class="card-body">
        <form enctype="multipart/form-data" id="addon-upload-form" action="{{ route('admin.system-setup.theme.install') }}"
              method="POST">
          @csrf
          <div class="d-flex flex-column gap-sm-20 gap-3">
            ...
            <div class="row g-4">
                ...
                <div class="bg-section rounded-8 p-sm-20 p-12">
                  <div class="file-upload-parent {{ $conditionOne && $conditionTwo ? '' : 'disabled' }}">
                    ...
                    <div id="file-upload-config"
                         data-icon-src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/icons/file-view.png') }}">
                    </div>

                    <div class="d--none progress-bar-container mt-4">
              ...
              <div class="col-lg-6">
                <div
                     class="bg-warning fs-12 text-dark rounded-8 h-100 d-flex justify-content-center flex-column bg-opacity-10 px-20 py-3">
                  <h3 class="text-info-dark">{{ translate('instructions') }}</h3>
                  <ul class="d-flex flex-column text-body m-0 gap-1 ps-20">
                    <li>
    ...
    <div class="card">
      <div class="card-body">
        <div class="mb-sm-20 d-flex justify-content-between align-items-center mb-3 flex-wrap gap-3">
          <div>
          ...
          @if (count($themes) < 2)
            <a class="btn bg-secondary lh-1 rounded-45 px-3 text-white" target="_blank"
        ...
        <div class="row g-3">
          ...
          @foreach ($themes as $key => $theme)
            @if (isset($theme['software_id']))
              <div class="col-sm-6 col-xl-4">
                <div
                     class="card h-100 {{ theme_root_path() == $key ? 'theme-active' : '' }} overflow-hidden border shadow-none">
                  <div class="bg-section p-sm-20 d-flex justify-content-between align-items-start gap-3 p-12">
                    <div>
                      <div class="d-flex align-items-center mb-3 gap-2">
                        ...
                        @if ($theme['is_active'])
                          <div class="fs-12 lh-1 fw-semibold bg-success rounded px-2 py-1 text-white">
                            {{ translate('Active') }}
                          </div>
                        @endif
                      </div>
                    ...
                    <div class="d-flex gap-sm-3 align-items-center gap-2">
                      @if ($key != 'default' && $key != 'theme_aster' && theme_root_path() != $key)
                        <button class="btn btn-outline-danger bg-danger icon-btn bg-opacity-10" data-bs-toggle="modal"
                  ...
                  <div class="card-body">
                    <div class="ratio-3-2 rounded-10 border">
                      <?php
          ...
          @if (count($themes) < 2)
            <div class="col-12">
              <div
                   class="rounded-10 p-sm-20 d-flex flex-column justify-content-center align-items-center gap-sm-20 h-100 gap-3 border border-dashed p-12">
                <img width="60" height="60"
                     src="{{ dynamicAsset(path: 'public/assets/new/back-end/img/explore-themes.png') }}" alt="">
  ...
  <span id="get-theme-publish-route" data-action="{{ route('admin.system-setup.theme.publish') }}"></span>
  <span id="get-theme-delete-route"></span>
  <span id="get-notify-all-vendor-route-and-img-src" data-csrf="{{ csrf_token() }}"
