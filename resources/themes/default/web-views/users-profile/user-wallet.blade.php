        ...
        <div class="card">
          <div class="card-body">
            ...
            <div class="d-flex align-items-center justify-content-between mb-3 gap-2">
              <h5 class="fs-16 m-0 font-bold">{{ translate('wallet') }}</h5>

              <button class="profile-aside-btn btn btn--primary d-lg-none rounded px-2 px-2 py-1">
            ...
            <div class="row g-0 g-md-3 h-100 gap-3">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                      <div class="card btn--primary h-100 position-relative mx-h-200">
                        <div class="card-body d-flex align-items-center z-2">
                          <div class="align-items-center d-flex justify-content-between w-100 flex-wrap gap-2 py-3">
                            ...
                            <h2 class="align-items-center d-flex fs-36 m-0 text-white">
                              ...
                              @if ($addFundsToWalletStatus)
                                <span class="fs-18 ml-2">
                                  <i class="tio-info-outined" data-toggle="tooltip" data-placement="bottom" title=""
                  ...
                  @else
                    <div class="col-md-5">
                      <div class="card btn--primary h-100 position-relative mx-h-200">
                        <div class="card-body d-flex align-items-center z-2">
                          <div class="d-flex justify-content-between align-items-center w-100 flex-wrap py-3">
                            <div class="text-white">
                              <p class="mb-0">{{ translate('wallet') }}</p>
                              <p class="mb-2">{{ translate('amount') }}</p>
                            </div>
                            ...
                            <h2 class="fs-36 d-flex align-items-center w-100 m-0 text-white">
                              {{ webCurrencyConverter(amount: $totalWalletBalance ?? 0) }}

                              @if ($addFundsToWalletStatus)
                                <span class="fs-18 ml-2">
                                  <i class="tio-info-outined" data-toggle="tooltip" data-placement="bottom"
                        ...
                        <div class="{{ count($addFundBonusList) > 1 ? 'owl-carousel add-fund-carousel' : '' }}">
                          @foreach ($addFundBonusList as $bonus)
                            <div class="item">
                              ...
                              <div class="add-fund-carousel-card z-1 w-100 rounded-10 ml-1 border p-4">
                                <div>
                                  <h4 class="text-accent mb-2">{{ $bonus->title }}</h4>
                                  <p class="text-dark mb-2">{{ translate('valid_till') }}
                                ...
                                <div>
                                  ...
                                  @else
                                    <p>{{ translate('add_fund_to_wallet') }}
                                      {{ webCurrencyConverter(amount: $bonus->min_add_money_amount) }}
                                      {{ translate('and_enjoy') }}
                                      {{ webCurrencyConverter(amount: $bonus->bonus_amount) }} {{ translate('bonus') }}
                                    </p>
                                  @endif
                                  <p class="fw-bold text-accent mb-0">
                                    {{ $bonus->description ? Str::limit($bonus->description, 40) : '' }}</p>
                                </div>
                                <img class="slider-card-bg-img add-fund-carousel-slider-card-bg" width="50"
                      ...
                      <div class="modal-body px-5">
                        ...
                        <form action="{{ route('customer.add-fund-request') }}" method="post">
                          ...
                          <div class="pb-4">
                            ...
                            <p class="text-center">{{ translate('add_fund_by_from_secured_digital_payment_gateways') }}
                            </p>
                            <input type="number"
                                   class="h-70 form-control text-24 rounded-10 fs-25-important light-placeholder text-center"
              ...
              <div class="col-md-12">
                <div class="">
                  <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-8 p-2">
                    <h6 class="fs-16 flex-grow-1 w-100 mb-0 font-bold">{{ translate('Transaction_History') }}</h6>
                      ...
                      <div class="dropdown flex-grow-1 w-100 d-flex justify-content-end">
                        <form action="{{ route('wallet') }}" method="get">
                          <button type="button" id="transactionFilterBtn"
                                  class="btn text-dark fs-14 d-flex align-items-center gap-10 rounded border px-3 py-1"
                          ...
                          <div
                               class="dropdown-menu dropdown-menu-end transaction-filter_dropdown dropdown-menu-end-0 shadow">
                            <div class="d-flex justify-content-between align-items-center border-bottom p-3">
                              <h5 class="fs-16 text-capitalize m-0 font-bold">
                                {{ translate('filter_data') }}
                              </h5>
                              <button id="filterCloseBtn" type="button"
                                      class="btn text-absolute-white rounded-circle lh-1 d-flex justify-content-center align-items-center m-0 border-0 p-0 pt-1">
                                <i class="fa fa-close"></i>
                              </button>
                            </div>
                            <div class="max-h-290px overflow-auto p-3">
                              <div class="mb-4">
                                <h6 class="text-capitalize fs-16 mb-3 font-bold">
                                  {{ translate('filter_by') }}
                                </h6>
                                <div class="d-flex transaction_filter_by gap-3">
                                  <label type="button"
                                         class="btn min-w-60px {{ $filterBy == '' || $filterBy == 'all' ? 'btn-outline-primary' : 'btn-outline-secondary' }} p-2">
                                    {{ translate('all') }}
                                    <input type="radio" name="filter_by" value="all"
                                           {{ $filterBy == '' || $filterBy == 'all' ? 'checked' : '' }}>
                                  </label>
                                  <label type="button"
                                         class="btn min-w-60px {{ $filterBy == 'debit' ? 'btn-outline-primary' : 'btn-outline-secondary' }} p-2">
                                    {{ translate('debit') }}
                                    <input type="radio" name="filter_by" value="debit"
                                           {{ $filterBy == 'debit' ? 'checked' : '' }}>
                                  </label>
                                  <label type="button"
                                         class="btn min-w-60px {{ $filterBy == 'credit' ? 'btn-outline-primary' : 'btn-outline-secondary' }} p-2">
                                    {{ translate('credit') }}
                                    <input type="radio" name="filter_by" value="credit"
                              ...
                              <div class="mb-4">
                                ...
                                <div class="position-relative">
                                  ...
                                  <input type="text" id="dateRangeInput" name="transaction_range"
                                         class="form-control" placeholder="{{ translate('Select_Date') }}"
                              ...
                              <div class="mb-4">
                                ...
                                <div class="d-flex flex-column transaction_earn_by gap-3">
                                  <label class="d-flex justify-content-between align-items-center">
                                    <span>{{ translate('Order_Transactions') }}</span>
                                    <input type="checkbox" class="earn-checkbox" name="types[]" value="order_place"
                                           {{ in_array('order_place', $transactionTypes) ? 'checked' : '' }}>
                                  </label>
                                  <label class="d-flex justify-content-between align-items-center">
                                    <span>{{ translate('Order_Refund') }}</span>
                                    <input type="checkbox" class="earn-checkbox border-dark" name="types[]"
                                           value="order_refund"
                            ...
                            <div class="card d-flex flex-row gap-3 p-3 shadow-lg">
                    ...
                    <div class="d-flex flex-column gap-2">
                      @foreach ($walletTransactionList as $key => $item)
                        @if ($item['admin_bonus'] > 0)
                          <div class="bg-light p-sm-3 d-flex justify-content-between g-2 my-1 rounded p-3">
                            <div class="">
                              <h6 class="d-flex align-items-center mb-2 gap-8">
                                <img src="{{ theme_asset(path: 'public/assets/front-end/img/icons/coin-success.png') }}"
                                     width="25" alt="">
                                <span class="absolute-ltr fs-18 font-bold">
                                  + {{ webCurrencyConverter(amount: $item['admin_bonus']) }}
                                </span>
                              </h6>
                              <h6 class="text-muted small text-capitalize fs-13 mb-0 font-semibold">
                                {{ ucwords(str_replace('_', ' ', translate('admin_bonus'))) }}
                              </h6>
                            </div>
                            <div class="small text-end">
                              <div class="text-muted text-capitalize mb-1 text-nowrap font-semibold">
                        ...
                        @endif
                        <div class="bg-light d-flex justify-content-between g-2 my-1 rounded p-3">
                          <div class="w-100">
                            <h6 class="d-flex align-items-center mb-2 gap-8">
                              @if ($item['debit'] != 0)
                          ...
                          <div class="small text-end">
                            <div class="text-muted text-capitalize mb-1 text-nowrap font-semibold">
                              {{ date('d M, Y H:i A', strtotime($item['created_at'])) }}</div>
                            @if ($item['debit'] != 0)
                  ...
                  @if ($walletTransactionList->count() == 0)
                    <div class="d-flex flex-column align-items-center my-5 gap-3 text-center">
                      <img width="72"
                      ...
                      <h6 class="text-muted mt-3">{{ translate('you_do_not_have_any') }}
                        <br> {{ request('type') != 'all' ? ucwords(translate(request('type'))) : '' }}
                        {{ translate('transaction_yet') }}
                      </h6>
                    </div>
                  @endif

                  <div class="card-footer mt-3 border-0 bg-transparent p-0">
                    @if (request()->has('type'))
