            ...
            if (response && response.data) {
                ...
                if (parseInt(cartCount) > 0) {
                    if ($headerBadge.length) {
                        ...
                        $headerBadge.attr('title', cartCount + ' items in cart');
                    } else {
                        // Add badge if it doesn't exist
                        $('a[href*="shop-cart"]').append('<span class="cart-count-badge absolute -top-2 left-3 flex h-5 w-5 items-center justify-center rounded-full bg-[#DC3545] text-xs font-bold text-white md:-top-3 md:left-4" title="' + cartCount + ' items in cart">' + cartCount + '</span>');
                    }
                } else {
