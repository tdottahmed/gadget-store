            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.customer_status == 0) {
                        location.reload();
                        toastMagic.warning(
                            $("#message-status-warning-text").data("text")
                        );
                    } else if (data.error) {
                        ...
                        location.reload();
                        toastMagic.success(
                            $("#message-status-success-text").data("text")
                        );
                    }
                },
            });
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success == 0) {
                        toastMagic.success(
                            $("#message-order-status-delivered-text").data(
                                "text"
                            )
                        );
                        location.reload();
                        ...
                        if (data.payment_status == 0) {
                            toastMagic.warning(
                                $("#message-order-status-paid-first-text").data(
                                    "text"
                                )
                            );
                            location.reload();
...
$("#choose_delivery_type").on("change", function () {
    ...
    $(".choose_delivery_man").hide();
    $("#by_third_party_delivery_service_info").hide();
    $("#steadfast-courier-btn").addClass("d-none");
    ...
    } else if (value === "stead_fast_courier") {
        $("#steadfast-courier-btn").removeClass("d-none");
    }
});
...
$("#addDeliveryMan").on("change", function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == true) {
                toastMagic.success(
                    $("#message-deliveryman-add-success-text").data("text")
                );
                location.reload();
            } else {
                toastMagic.error(
                    $("#message-deliveryman-add-error-text").data("text")
                );
            }
        },
        error: function () {
            toastMagic.error(
                $("#message-deliveryman-add-invalid-text").data("text")
            );
        },
    });
});
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == true) {
                ...
                toastMagic.error(
                    $("#message-deliveryman-charge-error-text").data("text")
                );
            }
        },
        error: function () {
            toastMagic.error(
                $("#message-deliveryman-charge-invalid-text").data("text")
            );
        },
    });
}
...
async function shippingAddressMap() {
    ...
    const map = new google.maps.Map(
        document.getElementById("location_map_canvas_shipping"),
        {
            center: {
                lat: latitude,
                lng: longitude,
...
async function billingAddressMap() {
    ...
    const { AdvancedMarkerElement } = await google.maps.importLibrary("marker");
    const map = new google.maps.Map(
        document.getElementById("location_map_canvas_billing"),
        {
...
async function locationShowingMap() {
    ...
    const { AdvancedMarkerElement } = await google.maps.importLibrary("marker");
    const map = new google.maps.Map(
        document.getElementById("location_map_canvas"),
        {
            center: {
                lat: latitude,
                lng: longitude,
